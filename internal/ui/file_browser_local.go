...
func (c *Controller) fileBrowserLocalList(dir string) (dirs []string, files []string, err error) {
	cfg := c.fileConfig()
...
func (c *Controller) fileBrowserLocalReadText(dir, name string, maxBytes int) (text string, truncated bool, size uint64, err error) {
	cfg := c.fileConfig()
	dir = strings.ReplaceAll(strings.TrimSpace(dir), "\\", "/")
