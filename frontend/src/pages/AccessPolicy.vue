  ...
  <section class="space-y-6">
    ...
    <section v-if="activeTab === 'current'" class="space-y-6">
      <section class="rounded-2xl border bg-card/90 p-5 text-card-foreground shadow-sm">
        ...
        <div class="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto_auto]">
          <div class="rounded-2xl border border-border/60 bg-background/70 px-4 py-3 text-sm text-muted-foreground">
            <p class="font-semibold text-foreground">{{ t("Authority Context") }}</p>
            <p class="mt-1">
              {{ t("Authority is resolved automatically from the current logged-in identity before loading or saving policy.") }}
            </p>
          </div>
          <div class="self-end">
            <Button
