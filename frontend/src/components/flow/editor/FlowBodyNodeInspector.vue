...
import {
  ...
  type FlowNodeDraft,
  type FlowNodeKind,
  type FlowSourceDraft,
  type FlowTransformExprMode,
  type NodeVisualFormModel,
  type VisualCompatibilityReason,
...
const normalizeSourceKind = (raw: string): FlowBindingSourceKind => {
  return bodyFlowBindingSourceKindOptions.find((kind) => kind === raw) ?? "trigger"
}
...
const createBranchCaseKey = () =>
  typeof crypto !== "undefined" && "randomUUID" in crypto
    ? crypto.randomUUID()
    : `body-branch-case-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`
        ...
        <div class="space-y-4">
          <div class="rounded-xl border border-border/70 bg-muted/20 p-4">
            <p class="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">
              {{ t("Body node authoring") }}
            </p>
            <p class="mt-2 text-sm text-muted-foreground">
              {{
                selectedNode.kind === "call"
                  ? t("Call nodes inside the nested body editor can reuse capability methods, ordinary fields, and bindings here.")
                  : t("Advanced body node kinds now reuse the same minimal form contracts as the root graph while the parent foreach body JSON remains the single source of truth.")
              }}
            </p>
          </div>
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div :id="inspectorFieldId('spec-mode-label')">
                ...
                <p class="mt-1 text-[11px] text-muted-foreground">
                  {{
                    supportsFormMode
                      ? t("Form mode edits the supported fields directly. Advanced JSON remains available when the current spec exceeds what ordinary mode can express.")
                      : t("This body node kind currently uses Advanced JSON authoring.")
                  }}
                </p>
              </div>
                ...
                <div class="rounded-xl border border-border/70 bg-muted/20 p-4">
                  ...
                  <div class="mt-4 grid gap-3 md:grid-cols-2">
                    <div>
                      ...
                      >
          <option v-for="kind in bodyFlowBindingSourceKindOptions" :key="kind" :value="kind">
        ...
        <option v-for="kind in bodyFlowBindingSourceKindOptions" :key="kind" :value="kind">
          {{ t(flowBindingSourceKindLabelKey(kind)) }}
              ...
              <div>
                ...
                />
                <p class="mt-1 text-[11px] text-muted-foreground">
                  {{ t("Nested foreach nodes keep their own body JSON inline here. Recursive visual body sessions are not available inside the body editor.") }}
                </p>
              </div>
            </div>
          ...
          <div v-if="selectedNode.specEditorMode === 'json'">
